...
MCP, the **Model Context Protocol**, is an open protocol that makes standard how applications provide context, tools and resources to Large Language Models. You best experience MCP through the use of [MCP Servers](https://modelcontextprotocol.io/docs/getting-started/intro) inside of your coding assistants (and other LLM power tools). In a practical sense they provide your tools additional context and functionality. From giving you the latest access to docs (like in the [AWS Docs](https://awslabs.github.io/mcp/servers/aws-documentation-mcp-server) MCP server) to allowing you to create and interact with databases (like with the [Neon](https://neon.com/docs/ai/neon-mcp-server) MCP Server). All in all they can be *critical* for productivity work.

...

So why are they being built in Python? 🤔

Don't get me wrong, making MCP servers in Python is a breeze. Check out this example using [FastMCP](https://github.com/jlowin/fastmcp):
```python
...
That's it. The MCP server is ready to accept MCP clients and do its thing! 🚀 
...
**Python 🐍:**
![python benchmark](/post-content/write-your-mcps-in-rust/python_benchmark.png)

**Rust 🦀:**
![python benchmark](/post-content/write-your-mcps-in-rust/rust_benchmark.png)
...
This example is of a MCP server called [Grimoire-MCP](https://github.com/darko-mesaros/grimoire-mcp) that does a bunch of filesystem operations in which it writes and reads files off a local disk. While the original was written in Rust I've used some [Kiro](https://kiro.dev/cli/) magic to have it converted into Python running with FastMCP. I've ensured that the feature parity is the same, and also that there are no non-optimal tasks being performed by the Python version.
...
![fastmcp splash](/post-content/write-your-mcps-in-rust/fastmcp_splash.png)

Okay, let's get building.

## Getting started with a Rust MCP server
...
![MCP Server diagram](/post-content/write-your-mcps-in-rust/mcp_server_diagram.png)

>**Please note:**
>
>You will need to have [this](https://github.com/darko-mesaros/ajde) CRUD API up and running locally. There is a quick start guide there that will get you through setting it up (you just need Rust, Docker, and just). Make sure to have this server running somewhere in the background on your local (or remote, do what you want) machine.
...
To install the required crates, make sure your `Cargo.toml` has the following dependencies:
```toml
anyhow = "1.0.100"
reqwest = "0.12.25"
...
Okay, let's add some scaffolding to our `main.rs` file. This will just set up some tracing for better debugging.
...
According to the [MCP Specification](https://modelcontextprotocol.io/specification/2025-11-25) the server offers the following features to the clients:
- **Tools:** Functions that your LLM can execute (ie something that invokes an API)
- **Resources:** Additional data and context (ie some stored docs)
- **Prompts:** Well ... Prompts. Stored prompts that the user can invoke.
...
- Setting up a Struct for the `TodoMcpServer`
- Implementing a `new()` function for that Struct
- Implementing `ServerHandler` with `get_info()` and `initialize()` for the `TodoMcpServer` struct

You may notice, there are **no tools** yet! That is fine, let's make sure this all works before adding any other complexities, such as tools.
...
![Compilation success](/post-content/write-your-mcps-in-rust/first_compiled.png)
...
Hopefully you've [installed](https://github.com/casey/just) `just` during the set up of the CRUD API. But if you haven't yet. Now is the perfect time as any. **Just** is a simple command runner, like the [good ol'](https://builder.aws.com/content/2gWap2Oti02cq9bAnDZnaDSrxdx/making-makefiles-for-fun-and-profit) `make` just made for the modern world.
...
mcp-test: release
  npx @modelcontextprotocol/inspector -e USER_ID={{ USER_ID }} ./target/release/crud_mcp
```

While the use of `just` is not mandatory for this tutorial, I find it way convenient to run complex commands with just ... `just`.

Back to it.

---

Whoa, what is that - a browser? 🫢 Yes, we are using the [MCP Inspector](https://modelcontextprotocol.io/docs/tools/inspector) tool to help us determine if our MCP server is actually doing what an MCP server should be doing.

![MCP Inspector](/post-content/write-your-mcps-in-rust/mcp_inspector_01.png)
...
![MCP Connected](/post-content/write-your-mcps-in-rust/mcp_connected.png)
...
The next bit, is us actually implementing this as a tool. Let's add a new tool to our `TodoMcpServer` tool router:
...
Looking at our new `get_all_todos()` function, let's discuss the MCP portions of this. **First off** the return of the function is a `CallToolResult` which is a specific [struct](https://docs.rs/rmcp/0.11.0/rmcp/model/struct.CallToolResult.html) that contains the content returned by the tools execution. **Next up**, we are running the `get_todos()` function we created before and converting the received Vector of Todos into `json`. We are also checking if any of those calls result in an `Err` if so, we return a `McpError::internal_error` back to the MCP client to let them know something went wrong. 
...
> **Note**:
>
> I fully understand that I am at first deserializing data from JSON then immediately serializing it back into JSON. This is just meant to illustrate how you would work with structs. Ultimately I would just make the `reqwest` here and return the JSON, OR better yet format the results back to the MCP client and only include the data it needs (not the entire JSON)
...
Let's try this again. Save your files and run `just mcp-test`. Back on the MCP Inspector, connect to the MCP and **List Tools**. Whoa, there is the `get_all_todos` tool! 🥳 Click on it and run the tool (did you remember to run the API in the background?). Huzzah!!! Success! 🚀

![MCP Inspector now with tools](/post-content/write-your-mcps-in-rust/mcp_inspector_02.png)
...
Does that make sense? It should, as it is very similar to the previous ones, except we have a helper struct where we new todo information that we will later serialize into JSON and send over to the API. Sweet! 🔥

### Setting up the Parameters struct
...
> We are using the [schemars](https://crates.io/crates/schemars) crate to generate JSON Schema documents
...
### The tool that takes parameters
...
Seen it? Good! Let's start with `[1]`: The function definition here takes the `NewTodoParameters` struct as a parameter but it also extracts the values into a `title` and `todo`. This is called **destructuring** and it is pure idiomatic Rust. This makes those values immediately available to the rest of the function.

---

#### Sidestep: Another way of extracting values

The [documentation](https://docs.rs/rmcp/0.11.0/rmcp/handler/server/wrapper/struct.Parameters.html) on `Parameters` shows that you just wrap the struct in a `Parameters<T>`, that works as well, but you just then have to handle extracting the values later on with a syntax I don't really like:
...
Now, on the MCP Inspector select the `create_new_todo` tool and give it a title and a body. Hit `Run Tool`:

![Creating new TODOs with the MCP](/post-content/write-your-mcps-in-rust/mcp_inspector_03.png)

Hooray! 🚀 You did it! Now, let's just wrap this up by testing it in an actual coding assistant.

## Kiro, meet my MCP

To test this out, I will be using [Kiro CLI](https://kiro.dev/cli/). When running Kiro CLI I edit my agent with the `/agent edit -n bloggy` command, and add the following to the `mcpServers` portion of the configuration:
```json
"mcp_crud": {
...
![MCP Server in action in Kiro CLI](/post-content/write-your-mcps-in-rust/kiro_mcp.png)
