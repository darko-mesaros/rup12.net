...
Okay, you've decided to dip your toes into Rust! Excellent choice! Oh and you want to build something in the cloud using AWS? How delightful! 👏 Wait what's that? You need the AWS SDK? Sure, that is not a problem, it's [right here](https://aws.amazon.com/sdk-for-rust/). However, there are a few things you need to know when it comes to using this SDK. Unlike the [Boto3 SDK](https://boto3.amazonaws.com/v1/documentation/api/latest/index.html) (the SDK for Python) there are a few additional steps you need to take before invoking those API calls. In this post let's explore how you can configure the AWS Rust SDK and how you can use that SDK with tools such as [LocalStack](https://www.localstack.cloud/).
...
<iframe
  width="560"
  height="315"
  src="https://www.youtube.com/embed/3nupUNtQ77k"
...
To work with the AWS SDK for Rust you need install the appropriate Rust crates. For *almost any* project you will work with you need to have [tokio](https://crates.io/crates/tokio), [aws-config](https://crates.io/crates/aws-config) (we will talk about it in a bit), and the actual AWS SDK [package you need](https://crates.io/search?q=aws-sdk). Unlike in Boto3 each service has its own SDK crate. So if you want to work with [AWS DynamoDB](https://aws.amazon.com/dynamodb/) you need the `aws-sdk-dynamodb` crate.

Here is an example of how I typically set up my new Rust project that will use the AWS SDK for some [Amazon S3](https://aws.amazon.com/s3/) shenanigans. This sets up all the needed packages and enables the required features:
```bash
cargo add tokio aws-config aws-sdk-s3 \
    --features tokio/full,aws-config/behavior-version-latest,aws-config/credentials-login
```

Let's modify our `main.rs` to set up the SDK and the Amazon S3 client:
```rust
...
> A quick note on `tokio`: The AWS SDK for Rust, requires an *async runtime*. Hence the need on the `full` feature flag. 

That's it. Yes! Ready to write to those S3 buckets! 🥳

## Configuring the Region and Credentials
...
```bash
cargo add tracing tracing_subscriber --features tracing_subscriber/env-filter
```

Here is what the modified `main.rs` should look like:
```rust
...
2025-12-22T19:54:31.057306Z  INFO aws_config::profile::credentials: loaded base credentials creds=Credentials { provider_name: "Login", access_key_id: "ASIXXXXXXXXXXXXXX", secret_access_key: "** redacted **", expires_after: "2025-12-22T20:06:20Z", account_id: "FAKEACCOUNTID", property_0: TypeErasedBox[Clone]:[CredentialsProfileLogin] }
```

Got it! 👏 We are using **us-west-2** as the region and it's using my **login** credentials. Both of these settings were obtained by the [new feature](https://docs.aws.amazon.com/signin/latest/userguide/command-line-sign-in.html) of `aws login`.
...
5. [EC2 IMDSv2](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/configuring-instance-metadata-service.html)

**🌍 Region:**
1. Environment variables
2. Shared config (`~/.aws/config`, `~/.aws/credentials`)
3. [EC2 IMDSv2](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/configuring-instance-metadata-service.html)

Let's make changes to what it uses. For this, we will be configuring the AWS Region by passing a command line parameter with [clap](https://crates.io/crates/clap) and by selecting a specific profile for our credentials. Please **note**, this does not make a lot of sense in a real world scenario, as you would likely get these parameters from the same place (ie the profile), but I'll be extra verbose here so you see the approaches. 
...
Now we pass this `region_provider` variable (a `RegionProviderChain`) to the `sdk_config` creation process by adding some additional parameters:
```rust
let sdk_config = aws_config::from_env()
    .region(region_provider) // <- Select the Region
    .load().await;
```
We have now selected the region. To expand this, and now select a different **credentials profile** let's add one more parameter:
```rust
let sdk_config = aws_config::from_env()
    ...
    .load().await;
```
Let's implement this in our `main.rs`, here is the fully modified file:
```rust
use tracing_subscriber::EnvFilter;
use clap::Parser;
...
cargo run "eu-west-1"
```
Barring having entered the wrong region name, or not having a given profile, you should get output similar to this:
...
2025-12-22T20:27:52.253169Z  INFO aws_config::profile::credentials: loaded base credentials creds=Credentials { provider_name: "Login", access_key_id: "AYYYYYYYYYYYYYYYY", secret_access_key: "** redacted **", expires_after: "2025-12-22T20:41:47Z", account_id: "OTHERFAKEACCOUNTID", property_0: TypeErasedBox[Clone]:[CredentialsProfileLogin] }
```

Huzzah! 🥳

## Calling the LocalStack endpoint

I'm gonna leave you with one more tip. Making a call to a different API endpoint. By default the AWS SDK makes the API calls to AWS (duh!), but what if you were doing some testing and local development with a tool like [LocalStack](https://www.localstack.cloud/)? Could you make those API calls to there? Absolutely yes!

I won't get into the details on how to use LocalStack, there are [excellent resources](https://docs.localstack.cloud/aws/getting-started/quickstart/) for that. But what I will show you is how to modify the `SdkConfig` object in the AWS SDK for Rust.
...
This will now target our local LocalStack instance. When using LocalStack, the region does not really matter. Ie the requests do not get routed to the defined region, and will always display results from `us-east-1`. I'll just add the following line to display all the buckets I list:
```rust
// Running any SDK call so the SDK would emit what credentials are used
